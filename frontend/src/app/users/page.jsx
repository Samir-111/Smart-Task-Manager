...
export default function UsersPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-200/90 shadow-premium-sm flex flex-col sm:flex-row gap-2.5 sm:gap-3 items-stretch sm:items-center justify-between">
        ...
        {searchQuery !== '' && (
          <button
            type="button"
            onClick={() => setSearchQuery('')}
            className="text-xs font-bold text-blue-600 hover:text-blue-800 px-2 py-1 hover:underline self-start sm:self-center"
          >
            Clear Search
          </button>
        )}
      ...
      {showAddModal && (
        <div className="bg-white rounded-2xl sm:rounded-3xl border border-blue-200 p-4 sm:p-6 shadow-premium animate-slide-up">
          <div className="flex items-center justify-between pb-3 mb-3.5 border-b border-slate-100">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-sm sm:text-base font-bold text-slate-900 tracking-tight">Create New User</h2>
          ...
          {errorMessage && (
            <div className="mb-3.5 p-3 bg-red-50/90 border border-red-200 rounded-xl flex items-start gap-2 text-xs text-red-700">
          ...
          {successMessage && (
            <div className="mb-3.5 p-3 bg-emerald-50/90 border border-emerald-200 rounded-xl flex items-start gap-2 text-xs text-emerald-700">
          ...
          <form onSubmit={handleCreateUser} className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            <div>
              ...
              <input
                ...
                required
                className="w-full px-3.5 py-2 rounded-xl border border-slate-200 text-xs sm:text-sm text-slate-900 bg-slate-50 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"
              />
            </div>
            ...
            <div className="sm:col-span-2 flex justify-end gap-2 pt-1">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="px-4 py-2 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-xs transition-all active:scale-98"
              >
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="p-4 rounded-2xl border border-slate-200 bg-white shadow-2xs space-y-2.5">
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/90 p-8 sm:p-12 text-center shadow-premium-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {filteredUsers.map((u) => {
            ...
            return (
              <div
                key={u.id}
                className={`p-3.5 sm:p-5 rounded-2xl sm:rounded-3xl border bg-white shadow-premium-sm flex flex-col justify-between transition-all duration-200 hover:-translate-y-0.5 ${
                  isCurrent
                ...
                <div>
                  <div className="flex items-start justify-between gap-2 mb-2 sm:mb-3">
                    <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                      <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 text-white flex items-center justify-center font-black text-xs sm:text-sm shrink-0 shadow-2xs">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {isCurrent && (
                            <span className="px-1.5 py-0.2 rounded-full text-[9px] font-bold bg-blue-100 text-blue-700 border border-blue-200">
                              Active
                            </span>
                          )}
                        </div>
                  ...
                  <div className="flex items-center justify-between mt-2 pt-2 sm:mt-3 sm:pt-3 border-t border-slate-100 text-xs">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-slate-600">
                      ...
                      <span className="font-bold text-slate-900 bg-slate-100 px-2 py-0.2 rounded-full text-[10px]">
                ...
                <div className="mt-3 pt-1.5 sm:mt-4 sm:pt-2">
                  {!isCurrent ? (
                    <button
                      type="button"
                      onClick={() => login(u)}
                      className="w-full py-1.5 sm:py-2 px-3 text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-600 hover:text-white rounded-xl border border-blue-100 transition-all duration-150 flex items-center justify-center gap-1.5 shadow-2xs active:scale-98"
                    >
                      <Sparkles className="w-3.5 h-3.5" />
                      <span>Switch to this user</span>
                    </button>
                  ) : (
                    <div className="w-full py-1.5 sm:py-2 text-center text-xs font-bold text-emerald-800 bg-emerald-50/90 rounded-xl border border-emerald-200 flex items-center justify-center gap-1.5">
