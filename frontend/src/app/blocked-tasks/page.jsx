...
export default function BlockedTasksPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Blocked Tasks</h1>
            <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-100/90 dark:bg-amber-950/60 text-amber-900 dark:text-amber-300 border border-amber-300/80 dark:border-amber-800/80 shadow-2xs">
      ...
      {loading ? (
        <div className="bg-white dark:bg-slate-900 p-8 sm:p-12 rounded-2xl sm:rounded-3xl border border-slate-100 dark:border-slate-800 text-center shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl border border-emerald-200/90 dark:border-emerald-900/50 p-8 sm:p-12 text-center shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
          ...
          <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Nothing is blocked</h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 mb-4 max-w-sm mx-auto">
            {searchQuery || priorityFilter !== 'ALL'
              ? 'No blocked tasks match your filters.'
              : 'All task dependencies are satisfied. You are clear to complete sprint deliverables!'}
          </p>
          <Link
            href="/all-tasks"
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 transition-colors shadow-2xs"
          >
        ...
        <div className="space-y-3 sm:space-y-4">
          {filteredTasks.map((task) => {
            return (
              <div
                key={task.id}
                className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl border border-amber-300/80 dark:border-amber-800/80 p-3.5 sm:p-5 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:border-amber-400 dark:hover:border-amber-700 transition-all duration-200"
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {task.assignedUser && (
                      <div className="mt-2 inline-flex items-center gap-1.5 text-[11px] text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2 py-0.5 rounded-full">
                  ...
                  <div className="min-w-0 md:w-80 p-3 rounded-xl sm:rounded-2xl bg-amber-50/50 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-900/60 flex flex-col justify-between">
                    <div>
                      <div className="flex items-center justify-between gap-1.5 mb-1">
                        <span className="text-[10px] font-extrabold uppercase tracking-wider text-amber-800 dark:text-amber-300 flex items-center gap-1">
                          <ShieldAlert className="w-3 h-3 text-amber-600 dark:text-amber-400" />
                          <span>Blocked → Prerequisite</span>
                        </span>
                        <span className="text-[10px] font-bold px-1.5 py-0.2 rounded-full bg-amber-100 dark:bg-amber-900/60 text-amber-900 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
                    ...
                    <div className="mt-2 pt-2 border-t border-amber-200/60 dark:border-amber-900/60 flex items-center justify-between gap-2">
                      <Link
                        href={`/edit-task/${task.dependsOnTaskId}`}
                        className="text-[11px] font-bold text-indigo-700 dark:text-indigo-400 hover:text-indigo-900 hover:underline flex items-center gap-1"
                      >
                        <span>Inspect</span>
                      ...
                      <div className="flex items-center gap-1">
                        <Link
                          ...
                          className="p-1 text-slate-400 hover:text-indigo-600 hover:bg-white dark:hover:bg-slate-800 rounded-lg transition-colors"
                          title="Edit"
                        >
                        ...
                        <button
                          ...
                          className="p-1 text-slate-400 hover:text-red-600 hover:bg-white dark:hover:bg-slate-800 rounded-lg transition-colors"
                          title="Delete"
                        >
