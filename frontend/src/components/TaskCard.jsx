...
export function TaskCard({
  ...
  return (
    <div
      className={`group relative overflow-hidden p-3.5 sm:p-5 rounded-2xl border transition-all duration-200 bg-white flex flex-col justify-between ${
        isBlocked
    ...
    >
      <div>
        ...
        <h3
          className={`font-bold text-xs sm:text-sm md:text-base leading-snug mb-1 transition-colors ${
            isDone ? 'text-slate-600 line-through opacity-85' : 'text-slate-900 group-hover:text-blue-950'
          }`}
        >
          {task.title}
        </h3>
      ...
      <div className="pt-2.5 border-t border-slate-100 flex items-center justify-between gap-2 mt-1">
        ...
        <div className="min-w-0 flex-1">
          {task.assignedUser ? (
            <div className="inline-flex items-center gap-1.5 text-xs text-slate-600 bg-slate-50 border border-slate-200/60 px-2 py-0.5 rounded-full max-w-[140px]">
              ...
              <span className="font-medium truncate text-[11px] text-slate-700">{task.assignedUser.name}</span>
            </div>
          ) : (
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          {!isDone && onComplete && (
            isAssignedToMe ? (
              ...
              <button
                ...
                className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-semibold bg-slate-100/80 text-slate-400 border border-slate-200/60 cursor-not-allowed opacity-60"
              >
          ...
          {isDone && (
            <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-600 py-0.5 px-2 bg-emerald-50/50 rounded-lg border border-emerald-100">
