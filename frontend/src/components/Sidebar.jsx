...
export function Sidebar({ isOpen, onClose }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 z-50 h-full w-[78%] sm:w-64 max-w-[320px] bg-[#0d1527] border-r border-slate-800/80 flex flex-col justify-between transition-transform duration-250 ease-out lg:translate-x-0 ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex flex-col flex-1 overflow-y-auto">
          {/* Brand Logo Header */}
          <div className="h-14 sm:h-16 px-4 sm:px-6 border-b border-slate-800/80 flex items-center justify-between shrink-0">
            <Link
              href="/"
              onClick={onClose}
              className="flex items-center gap-2.5 sm:gap-3 group"
            >
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-blue-600 text-white flex items-center justify-center font-black text-xs sm:text-sm shadow-md shadow-blue-500/30 group-hover:scale-105 transition-transform">
          ...
          <div className="px-2.5 sm:px-3.5 py-3 sm:py-4 flex-1 flex flex-col justify-between">
            <div>
              <p className="px-2.5 sm:px-3 text-[10px] font-extrabold uppercase tracking-wider text-slate-500 mb-1.5 sm:mb-2">
                Workspace Menu
              </p>
            ...
            <div className="mt-4 sm:mt-6 mx-0.5 sm:mx-1 p-2.5 sm:p-3 rounded-xl sm:rounded-2xl bg-slate-800/40 border border-slate-700/50">
